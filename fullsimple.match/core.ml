...
let is_matched patterns tmrecord = match tmrecord with
  | TmRecord(fi, fields) -> List.length fields = List.length patterns
  | _ -> false
...
let rec eval1 ctx t = match t with
  ...
  | TmLet(fi,x,t1,t2) ->
      let t1' = eval1 ctx t1 in
      TmLet(fi, x, t1', t2) 
  | TmPLet(fi, p, v1, t2) when (isval ctx v1) && (is_matched p v1) ->
      let m = terms v1 in List.fold_left (fun term v -> termSubstTop v term) t2 (List.rev m)
  | TmRecord(fi,fields) ->
