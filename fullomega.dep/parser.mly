...
Command :
  ...
  | UCID TyBinder
      { fun ctx -> ((Bind($1.i, $1.v, $2 ctx)), addname ctx $1.v) }
  | LCURLY UCID COMMA LCID RCURLY EQ Term           // {X, t} = {*X, t} as {Some X, T}
      { fun ctx ->
...
Type :
  ...
  | DEP LCID COLON Type DOT Type
      { fun ctx ->
          let ctx1 = addname ctx $2.v in
          TyDep($2.v,$4 ctx,$6 ctx1) }
  | RREF AType
      { fun ctx -> TyRef($2 ctx) }
