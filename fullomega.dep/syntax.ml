...
let tymap onvar c tyT = 
  ...
  | TyDep(tyX,tyT1,tyT2) -> TyDep(tyX,walk c tyT1,walk (c+1) tyT2)
  | TyVApp(tyT1,t2) -> TyVApp(walk c tyT1,t2)
  | TyString -> TyString
...
let rec printty_Type outer ctx tyT = match tyT with
  ...
  | TyDep(tyX,tyT1,t1) ->
      let (ctx1,tyX) = (pickfreshname ctx tyX) in
      obox(); pr "Dep "; pr tyX; pr "::"; printty_Type false ctx tyT1;
      pr ".";
      print_space ();
      printty_Type outer ctx1 t1;
      cbox()
...
and printty ctx tyT = printty_Type true ctx tyT 
...
and printtm ctx t = printtm_Term true ctx t 
