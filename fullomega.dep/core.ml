...
let rec formatty_Type ctx tyT = match tyT with
  | TyAbs(tyX,knK1,tyT2) ->
      ...
      "lambda " ^ x' ^ formatokn ctx knK1 ^ "." ^ formatty_Type ctx' tyT2
  | TyAll(tyX,knK1,tyT2) ->
      let (ctx1,tyX) = (pickfreshname ctx tyX) in
      "All " ^ tyX ^ formatokn ctx knK1 ^ ". " ^ formatty_Type ctx1 tyT2
  | TyDep(tyX,tyT1,tyT2) ->
      let (ctx1,tyX) = (pickfreshname ctx tyX) in
      "Dep " ^ tyX ^ ": " ^ formatty_Type ctx tyT1 ^ ". " ^ formatty_Type ctx1 tyT2
  | TyRef(tyT) -> "Ref " ^ formatty_AType ctx tyT
  | tyT -> formatty_ArrowType ctx tyT

and formatty_ArrowType ctx tyT = match tyT with 
  | TyArr(tyT1,tyT2) ->
      formatty_AppType ctx tyT1 ^ " -> " ^ formatty_ArrowType ctx tyT2
  | tyT -> formatty_AppType ctx tyT

and formatty_AppType ctx k = match k with 
  | TyApp(tyT1,tyT2) ->
      formatty_AppType ctx tyT1 ^ " " ^ formatty_AType ctx tyT2
  | TyVApp(tyT1,t2) ->
      formatty_AppType ctx tyT1 ^ " [" ^ formattm ctx t2 ^ "]"
...
and formatty_AType ctx tyT = match tyT with
  ...
  | TySome(tyX,knK1,tyT2) ->
      let (ctx1,tyX) = pickfreshname ctx tyX in
      "{Some " ^ tyX ^ formatokn ctx knK1 ^ "," ^ formatty_Type ctx1 tyT2 ^ "}"
  | TyBool -> "Bool"
  | TyNat -> "Nat"
  | TyId(b) -> b
  | TyFloat -> "Float"
...
and formatty ctx tyT = formatty_Type ctx tyT
...
and formattm_Term ctx t = match t with
  | TmAbs(fi,x,tyT1,t2) ->
      (let (ctx',x') = (pickfreshname ctx x) in
         "lambda " ^ x' ^ ":" ^ formatty_Type ctx tyT1 ^ "." ^ formattm_Term ctx' t2;
      )
  | TmTAbs(fi,x,knK,t) ->
      (let (ctx1,x) = (pickfreshname ctx x) in
            "lambda " ^ x ^ formatokn ctx knK ^ "." ^ formattm_Term ctx1 t;
            )
  | TmAssign(fi, t1, t2) ->
       formattm_AppTerm ctx t1 ^ " := " ^ formattm_AppTerm ctx t2;
  | TmLet(fi, x, t1, t2) ->
...
and formattm_AppTerm ctx t = match t with
  ...
  | TmRef(fi, t1) ->
       "ref " ^ formattm_ATerm ctx t1;
  | TmDeref(fi, t1) ->
       "!" ^ formattm_ATerm ctx t1;
...
and formattm ctx t = formattm_Term ctx t  
...
let rec computety ctx tyT = match tyT with
  | TyApp(TyAbs(tyX1,_,tyT12),tyT2) -> typeSubstTop tyT2 tyT12
  | TyVApp(TyDep(tyX1,_,tyT12),t2) -> typeSubstTop tyT12 tyT12
  | TyVar(i,_) when istyabb ctx i -> gettyabb ctx i
  | _ -> raise NoRuleApplies

and simplifyty ctx tyT =
  let tyT = 
    match tyT with
      | TyApp(tyT1,tyT2) -> TyApp(simplifyty ctx tyT1,tyT2)
      | TyVApp(tyT1,t2) -> TyVApp(simplifyty ctx tyT1,t2)
      | tyT -> tyT
  in 
  try
...
and tyeqv ctx tyS tyT =
  ...
  | (TyAll(tyX1,knK1,tyS2),TyAll(tyX2,knK2,tyT2)) ->
       let ctx1 = addname ctx tyX1 in
...
and kindof ctx tyT = match tyT with
  ...
  | TyApp(tyT1,tyT2) ->
      ...
      (match knK1 with
        ...
        | _ -> error dummyinfo ("arrow kind expected, received " ^ formatkn ctx knK1))
  | TyAll(tyX,knK1,tyT2) ->
      let ctx' = addbinding ctx tyX (TyVarBind knK1) in
...
and typeof ctx t =
  ...
  | TmAscribe(fi,t1,tyT) ->
    ...
    let tyT1 = typeof ctx t1 in
      if tyeqv ctx tyT1 tyT then
        tyT
      else
        error fi (
          "body of as-term does not have the expected type " ^
          formatty ctx (simplifyty ctx tyT) ^
          ", received " ^
          formatty ctx tyT1
        )
...
and evalbinding ctx store b = match b with
    TmAbbBind(t,tyT) ->
